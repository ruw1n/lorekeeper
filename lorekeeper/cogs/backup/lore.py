...
class LoreCog(commands.Cog):
    ...
    def _is_bio_question(self, q: str) -> bool:
        ...
        eventish = {
            "recent", "arcs", "major", "death", "dead", "died", "slain", "killed",
            "corpse", "body", "remains", "grave", "buried", "passing", "fate", "happened"
        }
        if any(t in eventish for t in toks):
            return False
    ...
    def _inline_source_links(self, text: str, rows: List[sqlite3.Row]) -> str:
        ...
        # 6) tidy spaces left behind
        out = re.sub(r"\s{2,}", " ", out)
        out = re.sub(r"\s+([.,;:!?])", r"\1", out)
        out = re.sub(r"\(\s+", "(", out)
        out = re.sub(r"\s+\)", ")", out)

        # 7) remove ugly doubled punctuation left by deleted placeholders
        out = re.sub(r",\s*,+", ", ", out)
        out = re.sub(r"\.\s*\.+", ".", out)

        # 8) normalize line endings a bit
        out = re.sub(r"\n{3,}", "\n\n", out).strip()
    ...
    async def lore_ask(self, ctx: commands.Context, *, question: str):
        ...
        if answer:
            ...
            answer = re.sub(r"\s*\[S\d+(?:\s*,\s*S\d+)*\]", "", answer)
            answer = re.sub(r"(?<!\w)S\d+(?:\s*,\s*S\d+)*(?!\w)", "", answer)
    ...
    def _intent_mode(self, question: str) -> str:
        """
