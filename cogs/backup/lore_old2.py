...
class LoreCog(commands.Cog):
    ...
    async def _fts_search(self, guild_id: int, query: str, limit: int = 8) -> List[int]:
        ...
        - Always returns up to `limit` unique message_ids, preserving "best-first" ordering.
        """
        ...
        # Build candidate queries (best → broader)
        ...
        def _prefixify(t: str) -> str:
            # Prefix matching helps with plural/singular and small variations (shadow/shadows/shadowy).
        ...
        # If phrase query: add human-friendly fallbacks
        if q_primary.startswith('"') and q_primary.endswith('"') and len(q_primary) > 2:
            ...
            if toks:
                if len(toks) >= 2:
                    # FTS5 NEAR is a function: NEAR(term1 term2, N)
                    ...
                    # cands.append(" OR ".join(toks))
                else:
                    cands.append(toks[0])

        # If AND query: add a prefixified variant (helps shadows vs shadow)
        ...
        # How many hits counts as "good enough" before we stop broadening
        ...
        async with self._db_lock:
            ...
            for i, cand in enumerate(cands):
                ...
                # If the primary query already produced enough, don't broaden further.
                if i == 0 and len(out) >= min_hits:
                    return out

            # Extra fallback: if primary was an AND query and we got nothing, try OR
    ...
    async def _openai_answer(self, *, question: str, excerpts: List[sqlite3.Row]) -> Optional[str]:
        ...
        prompt = (
            ...
            "Cover the FULL arc across time: early, middle, and recent developments.\n"
            "Call out turning points and how motivations/alliances changed.\n"
            "Cite sources using [S#] markers (example: [S12]). Do NOT write the word 'source'.\n\n"
            "Write the final answer using the format below. Be detailed.\n"
    ...
    async def _speaker_search_ids(self, guild_id: int, name: str, limit: int = 50, *, order: str = "DESC") -> List[int]:
        ...
        async with self._db_lock:
            ...
            rows = conn.execute(
                ...
                SELECT message_id
                FROM lore_messages
                WHERE guild_id=? AND LOWER(speaker_name)=LOWER(?)
                ORDER BY created_ts {order_sql}
                LIMIT ?
                """,
                (int(guild_id), name, int(limit)),
            ).fetchall()
        return [int(r[0]) for r in rows]
        ...
        *,
        ...
        async with self._db_lock:
            ...
            rows = conn.execute(
                ...
                SELECT message_id
                FROM lore_messages
                WHERE guild_id=? AND LOWER(speaker_name) LIKE ?
                ORDER BY created_ts {order_sql}
                LIMIT ?
                """,
    ...
    async def lore_peek(self, ctx: commands.Context, *, query: str):
        ...
        # 🔑 Shared retrieval (same as lore_ask)
        ...
        # Fetch anchor rows (preserve anchor order)
        ...
        # Apply the SAME post-processing as lore_ask so peek matches what ask fed the model
        is_bio = question.lower().startswith("who is ") or (
            len(self._guess_names_from_question(question)) == 1 and len(_fts_query_from_question(question).split()) <= 3
        )
        ...
        if mode == "timeline":
            ...
            if is_bio:
                ...
                rows_for_model = rows_for_model[-max_excerpts:]
        ...
        header = (
            ...
            f"| Window add: **{stats.get('window_add', 0)}** | Final rows: **{len(rows_for_model)}**\n"
    ...
    async def lore_ask(self, ctx: commands.Context, *, question: str):
        ...
        # Bio heuristic (keep your behavior)
        is_bio = question.lower().startswith("who is ") or (
            len(self._guess_names_from_question(question)) == 1 and len(_fts_query_from_question(question).split()) <= 3
        )
    ...
    def _rows_near_anchors(
        ...
        *,
        ...
        """
        Keep rows near anchors *within the same (channel_id, thread_id)*, not "near in global time order".
        This prevents cross-thread bleed when rows are interleaved by created_ts.
        """
    ...
    async def _retrieve_rows_for_question(
        ...
        *,
        ...
        is_bio = q.lower().startswith("who is ")
        looks_like_name = is_bio or (len(names) == 1 and len(re.findall(r"[a-z0-9']+", names[0])) <= 3)

        if looks_like_name:
            name_hint = q[7:].strip() if is_bio else names[0]
        ...
        rows = await self._fetch_messages(guild_id, ids)
        rows = sorted(rows, key=lambda r: int(r["created_ts"]))
        topic_anchors = set(int(x) for x in fts_anchor_ids) | set(int(x) for x in edge_ids)
        rows = self._rows_near_anchors(rows, topic_anchors, radius=radius, max_rows=max_excerpts)

        # 3) Topic focus (thread-local), unless bio query
        if (not looks_like_name) and rows and fts_anchor_ids:
            radius = int(os.getenv("LORE_TOPIC_CONTEXT_RADIUS", "3"))
            rows = self._rows_near_anchors(
                rows,
                set(int(x) for x in fts_anchor_ids),
                radius=radius,
