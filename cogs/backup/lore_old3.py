...
class LoreCog(commands.Cog):
    ...
    async def _backfill_channel(self, guild: nextcord.Guild, chan: nextcord.abc.GuildChannel, progress: BackfillProgress, *, force_full: bool = False):
        ...
        for t in threads:
            ...
            try:
                ...
                last_cursor_id = 0
                # Best-effort: archive the thread starter message (can be missing from Thread.history)
                try:
                    ...
                    try:
                        ...
                        for attr in ("message_id", "starter_message_id"):
                            mid = getattr(t, attr, None)
                            if mid:
                                starter_ids.append(int(mid))
                        starter_ids.append(int(t.id))  # fallback (sometimes works)

                        for starter_id in starter_ids:
                            try:
                                # thread starter lives in the PARENT channel for message-threads
                                for starter_id in starter_ids:
                                    starter = None

                                    # try parent channel first
                                    if hasattr(chan, "fetch_message"):
                                        try:
                                            starter = await chan.fetch_message(starter_id)
                                        except Exception:
                                            starter = None

                                    # fallback: try fetching from the thread itself
                                    if starter is None and hasattr(t, "fetch_message"):
                                        try:
                                            starter = await t.fetch_message(starter_id)
                                        except Exception:
                                            starter = None

                                    if starter is not None:
                                        try:
                                            saved = await self._archive_message(starter, progress=progress)
                                            if saved:
                                                progress.msgs_saved += 1
                                        except Exception:
                                            pass
                ...
                if head_n > 0:
                    try:
                        async for m in t.history(limit=head_n, oldest_first=True, after=None):
                            if not m.guild:
                                continue
                            progress.msgs_seen += 1
                            try:
                                ...
                                pass
                            saved = await self._archive_message(m, progress=progress)
                            if saved:
                                progress.msgs_saved += 1
                    except Exception:
                        pass    
            ...
            for cid in ids:
                ...
                if isinstance(ch, nextcord.Thread):
                    ...
                    try:
                        ...
                        # Best-effort: archive the thread starter message (can be missing from Thread.history)
                        try:
                            # Best-effort: archive the thread starter message.
                            try:
                                parent = getattr(ch, "parent", None)
                                starter_ids = []
                                for attr in ("message_id", "starter_message_id"):
                                    mid = getattr(ch, attr, None)
                                    if mid:
                                        starter_ids.append(int(mid))
                                starter_ids.append(int(ch.id))
                                ...
                                if parent and hasattr(parent, "fetch_message"):
                                    for starter_id in starter_ids:
                                        try:
                                            ...
                                            saved = await self._archive_message(starter, progress=p)
                                            if saved:
                                                p.msgs_saved += 1
                                            break
                                        except Exception:
                            ...
                            # Fallback: try fetching within the thread
                            try:
                                starter = await ch.fetch_message(int(ch.id))
                                await self._archive_message(starter, progress=p)
                            except Exception:
                                pass
                        except Exception:
                            pass
                        ...
                        if head_n > 0:
                            try:
                                async for m in ch.history(limit=head_n, oldest_first=True, after=None):
                                    if not m.guild:
                                        continue
                                    p.msgs_seen += 1
                                    try:
                                        p.last_msg_ts = int(m.created_at.timestamp())
                                    except Exception:
                                        pass
                                    try:
                                        saved = await self._archive_message(m, progress=p)
                                        if saved:
                                            p.msgs_saved += 1
                                    except Exception:
                                        pass
    ...
    async def lore_ask(self, ctx: commands.Context, *, question: str):
        ...
        # After time_stratified_rows(...) and before question_for_model / webhook preference:
        if _anchors:
        ...
        # Don't let query-control prefixes bleed into the LLM question
        question_for_model = re.sub(r"(?i)^(exact|phrase)\s*:\s*", "", question).strip()

        # Bio query guard: only keep excerpts that actually mention the subject,
        # otherwise the model will mis-attribute nearby facts.
        ...
        if is_bio:
            ...
            if subj_l:
                ...
                for r in rows:
                    ...
                    if (subj_l in who) or (subj_l in txt):
                        focused.append(r)
        ...
        # Prefer RP/webhook excerpts so infodumps don't hijack definitions
        ...
        for r in rows:
            if str(r["speaker_type"]) == "webhook" or int(r["webhook_id"] or 0) != 0:
                ...
                usr.append(r)
        if wh:
            combined = wh + usr[-max_user_rows:]
            by_id = {int(r["message_id"]): r for r in combined}
            rows = sorted(by_id.values(), key=lambda r: int(r["created_ts"]))

        answer = await self._openai_answer(question=question_for_model, excerpts=rows)
        if answer:
